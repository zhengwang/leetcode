 ...
 * @return {number}
 */
SortAlgorithm.prototype.binarysearch = function(nums, lo, hi, key) {
	if (lo >= hi) return nums[lo] < key ? lo + 1 : lo;
	const mid = lo + Math.floor((hi - lo) / 2);
	if (nums[mid] < key) {
		// go right
		return this.binarysearch(nums, mid + 1, hi, key);
	} else if (nums[mid] > key) {
		return this.binarysearch(nums, lo, mid - 1, key);
	} else {
		return mid;
	}
}

SortAlgorithm.prototype.insertsort_bs = function() {
	let arr = this.nums.slice();
	const LEN = arr.length; 
	
	for(let i=1; i<LEN;i++) {
		let X = arr[i];
		let pos = this.binarysearch(arr, 0, i-1, X);
		let j = i-1;
		while(j>=pos) {
			arr[j+1]=arr[j];
			j--;
...
algorithm.insertsort_bs();
// let nums = [1,2,3,4];
// console.log(algorithm.binarysearch(nums, 0, nums.length-1, 4));